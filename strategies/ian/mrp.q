...
mrp:{
  ...
  prices:exec gld:close,gdx:close_gdx by date from (select date,close from daily where sym=`GLD) 
       lj`date xkey(select date, close_gdx:close from daily where sym=`GDX); /is this needed??
  ...
  t_prices:update spread:gld-first[h]*gdx from t_prices;
