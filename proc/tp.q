{
 if[not .qi.exists p:.qi.path(.paths.stack;`names;.conf.name;`tables.txt);
 ...
 {[t] set[t].schema.t t}each vt;
 }[]

\d .u

LOGDIR:.qi.path(.conf.tplogs;.conf.name)
...
tick:{init[];if[not min(`time`sym~2#key flip value@)each t;'`timesym];@[;`sym;`g#]each t;d::.z.D;;L::` sv .u.LOGDIR,`$"tp",string d;l::ld d};
